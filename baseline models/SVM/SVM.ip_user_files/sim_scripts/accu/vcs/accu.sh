...
#
# Script generated by Vivado on Wed Jan 08 16:41:53 +0800 2025
...
#
# Usage        : accu.sh
#                accu.sh [-lib_map_path] [-step] [-keep_index] [-noclean_files]*
#                accu.sh [-reset_run]
#                accu.sh [-reset_log]
#                accu.sh [-help]
#
...
#                script (type 'accu.sh -help' for more information).
#
...
# script info
echo -e "accu.sh - Script generated by export_simulation (Vivado v2023.1 (64-bit)-id)\n"
...
run()
{
  ...
  if [[ ($b_step == 1) ]]; then
    case $step in
      ...
      ;;
      * )
        echo -e "ERROR: Invalid or missing step '$step' (type \"./accu.sh -help\" for more information)\n"
        exit 1
      esac
  else
    init_lib
...
elaborate()
{
  vcs $vcs_elab_opts xil_defaultlib.accu xil_defaultlib.glbl -o accu_simv
}

# RUN_STEP: <simulate>
simulate()
{
  ./accu_simv $vcs_sim_opts -do simulate.do
}
...
reset_run()
{
  files_to_remove=(ucli.key accu_simv vlogan.log vhdlan.log compile.log elaborate.log simulate.log .tmp_log .vlogansetup.env .vlogansetup.args .vcs_lib_lock scirocco_command.log lib_sc.so 64 AN.DB csrc accu_simv.daidir vcs_lib c.obj)
...
check_arg_value()
{
  ...
  if [[ ($1 == "-lib_map_path") && ($2 == "") ]];then
    echo -e "ERROR: Simulation library directory path not specified (type \"./accu.sh -help\" for more information)\n"
    exit 1
  fi
}
...
usage()
{
  msg="Usage: accu.sh [-help]\n\
Usage: accu.sh [-step]\n\
Usage: accu.sh [-lib_map_path]\n\
Usage: accu.sh [-reset_run]\n\
Usage: accu.sh [-reset_log]\n\
Usage: accu.sh [-keep_index]\n\
Usage: accu.sh [-noclean_files]\n\n\
