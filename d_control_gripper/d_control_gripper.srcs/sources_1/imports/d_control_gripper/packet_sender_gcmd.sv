...
always @(posedge sys_clk or negedge sys_rst_n) begin
    if(sys_rst_n == 1'b0) begin
        ...
        sending <= 1'b1;
    end else if(sending && tx_busy == 1'b0 && byte_idx == cmd_len) begin
        sending <= 1'b0;
    end
end
...
always @(posedge sys_clk or negedge sys_rst_n) begin
    if(sys_rst_n == 1'b0) begin
        byte_idx <= 10'b0;
    end else if(sending && tx_busy == 1'b0 && uart_flag == 1'b0 && byte_idx < cmd_len) begin
        byte_idx <= byte_idx + 1'b1;
    end else if(uart_flag && byte_idx == cmd_len) begin
        byte_idx <= 10'b0;
    end
end
