            ...
            always @(posedge sys_clk or negedge sys_rst_n) begin
                if(sys_rst_n == 1'b0) begin
                    ...
                    case(rawKeys[i][j][7:3])
                        ...
                        5'b0_0001: keys[i][j] <= 'd2;
                        5'b0_0010: keys[i][j] <= 'd3;
                        5'b0_0011: keys[i][j] <= 'd4;
                        5'b0_0100: begin
                            if(rawKeys[i][j][2])
                                keys[i][j] <= 'd6;
                            else
                        ...
                        5'b0_0101: keys[i][j] <= 'd7;
                        5'b0_0110: keys[i][j] <= 'd8;
                        5'b0_0111: keys[i][j] <= 'd9;
                        5'b0_1000: begin
                            if(rawKeys[i][j][2])
                                keys[i][j] <= 'd11;
                            else
                        ...
                        5'b0_1001: keys[i][j] <= 'd12;
                        5'b0_1010: keys[i][j] <= 'd13;
                        5'b0_1011: keys[i][j] <= 'd14;
                        5'b0_1100: begin
                            if(rawKeys[i][j][2])
                                keys[i][j] <= 'd16;
                            else
                        ...
                        5'b0_1101: keys[i][j] <= 'd17;
                        5'b0_1110: keys[i][j] <= 'd18;
                        5'b0_1111: keys[i][j] <= 'd19;
                        5'b1_0000: keys[i][j] <= 'd20;
                        default  : keys[i][j] <= 'd20;
                    endcase
                end
            end
        end
...
always @(posedge sys_clk or negedge sys_rst_n) begin
    if(sys_rst_n == 1'b0) begin
        spa_en <= 1'b0;
    end else if(flag_in || (NEcnt == NE_NUM-1 && Ncnt < N-1)) begin
        spa_en <= 1'b1;
...
always @(posedge sys_clk or negedge sys_rst_n) begin
    if(sys_rst_n == 1'b0) begin
        Ncnt <= 4'b0;
    end else if(flag_in || (Ncnt == N-1 && NEcnt == NE_NUM-1)) begin
        Ncnt <= 4'b0;
    end else if(NEcnt == NE_NUM-1) begin
        Ncnt <= Ncnt + 1'b1;
    end
...
always @(posedge sys_clk or negedge sys_rst_n) begin
    if(sys_rst_n == 1'b0) begin
        ...
        chan_addr <= CHcnt;
        fea_addr1 <= keys[CHcnt][Ncnt];
    end else if(graph_en) begin
        if(NEcnt < 'd10) begin
            chan_addr <= CHAN_NUM;
            if(NEcnt < 'd4) begin
                fea_addr1 <= keys[GRAPH[NE_NUM-1-NEcnt][0]][Ncnt];
            end else begin
                fea_addr1 <= keys[GRAPH[NE_NUM-1-NEcnt][0]][Ncnt];
                fea_addr2 <= keys[GRAPH[NE_NUM-1-NEcnt][1]][Ncnt];
            end
    ...
    for(i=0; i<D; i=i+1) begin
        always @(posedge sys_clk or negedge sys_rst_n) begin
            if(sys_rst_n == 1'b0) begin
                ...
                if(gint_HD[i] + sint_HD[i] < ((30+9*rot_index-1)/2 + 1)) begin
                    gsbin_HD[i] <= 1'b0;
                end else begin
                    gsbin_HD[i] <= 1'b1;
                end
...
always @(posedge sys_clk or negedge sys_rst_n) begin
    if(sys_rst_n == 1'b0) begin
        rot_index <= 4'b0;
    end else if(flag_in || (rot_index == N-1 && tsHV_flag)) begin
        rot_index <= 4'd0;
    end else if(tsHV_flag) begin
        rot_index <= rot_index + 1'b1;
    end
end
...
always @(posedge sys_clk or negedge sys_rst_n) begin
    if(sys_rst_n == 1'b0) begin
        HV <= 'b0;
    end else if(temp_flag && rot_index == 1'b0) begin
        // HV <= (gsbin_HD << (N-1-rot_index)) | gsbin_HD >> (D-(N-1-rot_index)) & ((1'b1 << (N-1-rot_index)) - 1'b1);
        HV <= gsbin_HD;
    end else if(temp_flag) begin
        // HV <= HV ^ ((gsbin_HD << (N-1-rot_index)) | gsbin_HD >> (D-(N-1-rot_index)) & ((1'b1 << (N-1-rot_index)) - 1'b1));
        HV <= ~(({HV[0], HV[D-1:1]}) ^ gsbin_HD);
    end
end
...
always @(posedge sys_clk or negedge sys_rst_n) begin
    if(sys_rst_n == 1'b0) begin
        HV_flag <= 1'b0;
    end else if(temp_flag && rot_index == N-1) begin
        HV_flag <= temp_flag;
    end else begin
        HV_flag <= 1'b0;
    end
